...
static esp_gatt_if_t gatts_if_global = 0; //Richards påhitt
static uint16_t client_conn = 0; //Richards påhitt
static uint8_t recieved_data = 0x00; //Richards påhitt
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            ESP_LOG_BUFFER_HEX(CLIENT_TAG, param->write.value, param->write.len);
